...
Two honest limits on that. The route *table* is tested and the route *handlers* are not: they still live inline in `server/index.ts`, which no test imports, so what is covered is the matching and the shape of the surface, not the bodies. And the surface-as-data is not the whole surface: the table holds 30 route entries, while seven further paths — push and alerts — remain a small if-chain in `alerts/manager.ts`, reached before the table and invisible to the surface test.
