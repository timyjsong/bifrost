...
**How this history relates to the real one.** Bifrost is developed in a private repository and published here as a filtered extraction: internal working notes, a build-state log, ticket files and audit write-ups are excluded, and machine identifiers are replaced with placeholders — a claim `server/placeholders.test.ts` enforces rather than asserts, over published history and not just the working tree, because earlier versions of it were true of the tree and false of the history. Two consequences are visible from the outside and worth naming rather than leaving you to trip over them. A few early commit messages mention files you won't find in their diffs, because those files were filtered out of the published history. And Build 2 lands as one large commit instead of the per-milestone series Builds 0 and 1 got — it was extracted in a single pass, not written in one sitting, and its design doc predates its code by weeks even though the commits arrive in the opposite order.
